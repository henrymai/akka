    ...
    new JavaTestKit(system) {
...

      {
        ...
        RunnableGraph.fromGraph(FlowGraph.create(builder -> {
          ...
          builder.from(bcast).toInlet(builder.add(droppySink(mySink1, 10)));
          builder.from(bcast).toInlet(builder.add(droppySink(mySink2, 10)));
          builder.from(bcast).toInlet(builder.add(droppySink(mySink3, 10)));
